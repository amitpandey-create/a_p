# streamlit_sales_management_app.py
# Streamlit Sales Management App with Authentication (MongoDB-backed)
# Includes a dependency guard for pymongo so the app shows a friendly error when pymongo is missing.
# ------------------------------------------------------------------------------

# --- Dependency guard for pymongo ------------------------------------
...
try:
    ...
    # Render a friendly error inside Streamlit and stop further execution.
    try:
        st.set_page_config(page_title="Sales Management App — Missing Dependency", layout="centered")
    except Exception:
        pass
    st.title("Sales Management App")
    st.error("Required dependency `pymongo` is not installed or failed to import.")
    st.markdown("**Import error:**")
    st.code(str(e))
    st.write("To fix this, install `pymongo` in the same Python environment your app runs in.")
    st.markdown("**Commands:**")
    st.code("pip install pymongo")
    st.markdown("If your environment uses a different pip (e.g. `pip3` or virtualenv), use that instead:")
    st.code("python -m pip install pymongo")
    st.write("---")
    st.info("After installing, restart the Streamlit app (`streamlit run streamlit_sales_management_app.py`).")
    # Optional: let user try to re-import if they installed pymongo via another terminal
    if st.button("Try to continue (re-attempt import)"):
        try:
            import importlib
            importlib.invalidate_caches()
            import pymongo  # attempt import again
            st.success("Import succeeded. Press Rerun if the app doesn't auto-reload.")
        except Exception as e2:
            st.error("Import still failing. Check installation in your terminal.")
            st.code(str(e2))
    st.stop()

# If we reach here, pymongo (and bson) imported OK — continue with the rest of the app
# ------------------------------------------------------------------------------
